...
export const commentById = async (id: string): Promise<IComment> => {
  const comment = await Comment.findById(id);
  return comment;
};

export const createComment = async (comment: string, authorId: string, postId: string): Promise<IComment> => {
