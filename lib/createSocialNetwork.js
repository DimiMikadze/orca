...
const createApp = () => {
  ...
  updatePackageJsonName(`${projectName}/api/package.json`, `${projectName}-api`);
  updatePackageJsonName(`${projectName}/frontend/package.json`, `${projectName}-frontend`);
  ...
  if (packageManager === "npm") {
    ...
    spawn.sync("npm", ["install"], { cwd: `${projectName}/api`, stdio: "inherit" });
    spawn.sync("npm", ["install"], { cwd: `${projectName}/frontend`, stdio: "inherit" });
  } else {
  ...
  fs.renameSync(`${projectName}/frontend/.env.example`, `${projectName}/frontend/.env`);

  // Remove .git and lib folder
  ...
  console.log(
    `To start the application, ${chalk.cyan(
      "cd"
    )} inside the newly created project and run ${chalk.cyan(packageManager + " start")} `
  );
  console.log();
