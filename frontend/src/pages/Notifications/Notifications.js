...
const Notifications = () => {
  ...
  const variables = { userId: auth.user.id, skip: 0, limit: NOTIFICATIONS_PAGE_NOTIFICATION_LIMIT };

  return (
    <Root maxWidth="md">
      <Head title={`${auth.user.username}'s Notifications`} />

      <Query query={GET_USER_NOTIFICATION} variables={variables} notifyOnNetworkStatusChange>
        {({ data, loading, fetchMore, networkStatus }) => {
          ...
          if (!notifications.length) return <Empty text="No notifications yet." />;
